...
- **ROMS**
    ...
    - all files containing `_ROMS_` are the detrended ROMS data, which is the model output from the WGOA ROMS model.
    the files are for 300 m and 1000 m depth, and contain the data for the entire WGOA region.
    ppS and ppL are primary production for small and large copepods.
    - The folder **ROMSOutputWGOA** contains all the ROMS outputs in long and wide formats, it also has files for production anommaly.
    The wide files contain the temperatures for each distinct ssps (126, 245 and 585). 
...
- **AKFIN**
    ...
    - `wgoa_catch` files are also from AKFIN, they contain both fish ticket (mainly state) and CAS (federally managed) data. 
    The file `_1990_mean.csv` is the mean between years 1990-1993, the baseline inputs for our Ecopath model. 
    The files are also in wide and long formats.
