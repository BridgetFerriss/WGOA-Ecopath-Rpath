...
for (depth in depth_list) {
  ...
  #### Set area based on depth ####
  if (depth == 300) {
    area_df <- data.frame(
      NMFS_AREA = c("610", "620", "630", "640", "650"),
      area_m2   = c(57225003746, 62597059226, 98582220025, 32560976631, 36726651409)
    )
  } else if (depth == 1000) {
    area_df <- data.frame(
      NMFS_AREA = c("610", "620", "630", "640", "650"),
      area_m2   = c(63986698621, 69583703140, 105918077937, 37270389681, 43952466109)
    )
  }
  ...
  wgoa_summary <- wgoa_summary %>%
    group_by(varname, simulation, year) %>%
    summarise(mean_biomass = mean(biomass_tonnes), .groups = 'drop')
  
  # Write biomass output
  write.csv(wgoa_summary,
            paste0("WGOA_source_files/ROMSOutputWGOA/Long_WGOA_B_summary_", depth, "_v2_corrected.csv"),
            row.names = FALSE)
  ...
  write.csv(prod_data, paste0("WGOA_source_files/ROMSOutputWGOA/Long_WGOA_prod_", depth, ".csv"), row.names = FALSE)
  ...
  roms_avg_data <- bind_rows(nep_hind_avg, nep_hist_avg, nep_ssp126_avg, nep_ssp245_avg, nep_ssp585_avg) %>%
    mutate(date = as_date(date),
           month = month(date),
           year = year(date))
  ...
  # Filter and compute area-weighted means
  ...
  write.csv(temp_weighted, paste0("WGOA_source_files/ROMSOutputWGOA/Long_WGOA_temp_monthly_", depth, ".csv"), row.names = FALSE)
  write.csv(temp_annual, paste0("WGOA_source_files/ROMSOutputWGOA/Long_WGOA_temp_annual_", depth, ".csv"), row.names = FALSE)
  
  cat("Processing complete for depth:", depth, "m\n")
}
...
for (depth in depth_list) {
  ...
  temp_weighted <- read.csv(paste0("WGOA_source_files/ROMSOutputWGOA/Long_WGOA_temp_monthly_", depth, ".csv"))
  temp_annual <- read.csv(paste0("WGOA_source_files/ROMSOutputWGOA/Long_WGOA_temp_annual_", depth, ".csv"))
 ...
 # ggsave(paste0("WGOA_source_files/ROMSOutputWGOA/WGOA_temp_time_", depth, ".png"), p_temp_time, width = 10, height = 6)
 ...
 # ggsave(paste0("WGOA_source_files/ROMSOutputWGOA/WGOA_temp_annual_", depth, ".png"), p_temp_annual, width = 10, height = 6)
