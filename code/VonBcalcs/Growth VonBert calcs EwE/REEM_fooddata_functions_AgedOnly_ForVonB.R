#BF modified REEM_fooddata_functions_LW.r to 
# 1)add length_mm and weight_g (matching original units in RACEBASE for length weight regression) Jan2023
# 2) to remove any specimens that were not aged need length and age for von Bertallanffy growth function for EwE
...
get_lw <- function(predator="P.cod", model="EBS", years=NULL, all.data=F){
  ...
  x <- model_haul %>%
    ...
    mutate(predator = predator_name,
           length_cm = length/10, length_mm=length, weight_g=weight) %>% relocate(predator) #BF added length_mm and weight_g
  ...
  lw <- lm(log(weight_g) ~ log(length_mm), data=x)
  loglw_a <- lw$coef[[1]]
  lw_b <- lw$coef[[2]]
  
  dat <- x %>%
    mutate(lbin = as.character(cut(length_mm, ppar$LCLASS, right=F)),
           log_lw_a = lw$coef[[1]], 
           lw_b = lw$coef[[2]], 
           log_diff = log(weight_g) - (loglw_a + lw_b*log(length_mm)))
