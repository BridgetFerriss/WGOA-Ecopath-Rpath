...
# We can also write it for oracle, and have both download methods available, 
# but I will need help with the oracle part.
#------------------------------------------------------------------------------#
...
# Before: race_lookup_col was already defined as:
 race_lookup_col <- c(
   "EBS"  = "ebs_ecopath", #here you have to see if the lookup table matches the name. We might need an updated lookup table. 
   "AI"   = "ai_ecopath",
   "WGOA" = "final_wgoa",
   "EGOA" = "final_egoa"
 )
...
for (p in pred_names) {
  ...
  pred_params[[p]]$vonb <- list(h= pdat$vb_kLinf= pdat$vb_linf_mm, t0= pdat$vb_t0,
