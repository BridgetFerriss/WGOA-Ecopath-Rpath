...
test1 <- cpue %>% filter(YEAR==2023 & STRATUM==11)
test2 <- get_cpue_all(model = this.model) %>%
  filter(year==2023 & stratum_bin==11 & species_code==21740)
...
stratsum <- get_cpue_all(model = this.model) %>%
  ...
  mutate(
    ...
    var_wtcpue    = (tot_wtcpue2  - (tot_wtcpue * tot_wtcpue)/stations)/(stations-1),
