...
wgoa_catch1 <- dbGetQuery(con, paste0("with dat as (select 
year,
species_group_code,
species_group_name,
species_name,
...
wgoa_catch2 <- dbGetQuery(con, paste0("with dat as (select 
year,
species_group_code,
species_group_name,
species_name,
...
wgoa_catch3 <- dbGetQuery(con, paste0("with dat as (select 
year,
species_group_code,
species_group_name,
species_name,
...
wgoa_catch4 <- dbGetQuery(con, paste0("with dat as (select 
year,
species_group_code,
species_group_name,
sum(weight_posted) catch_mt,
...
agency_species <-  dbGetQuery(con, paste0("select * from akr.agency_specie where agency = 'AKR'")) %>%
  rename_with(tolower)

# save
# sorry for the long species names, you can rename whichever of these you use to whatever you want. 
write.csv(wgoa_catch1, paste0("data/",end_year,"/wgoa_catch_year_group_spec_ret_gear.csv"), row.names=FALSE)
write.csv(wgoa_catch2, paste0("data/",end_year,"/wgoa_catch_year_group_spec_ret.csv"), row.names=FALSE)
write.csv(wgoa_catch3, paste0("data/",end_year,"/wgoa_catch_year_group_spec.csv"), row.names=FALSE)
write.csv(wgoa_catch4, paste0("data/",end_year,"/wgoa_catch_year_group.csv"), row.names=FALSE)
write.csv(species_group_content, "data/metadata/akr_species_group_content.csv", row.names=FALSE)
write.csv(agency_species, "data/metadata/akr_species_code_lookup.csv", row.names=FALSE)
